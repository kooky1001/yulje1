...
$(function(){
	...
	$.ajax("/listClinic.ajax",{success:function(data){
		...
		$.each(data, function(idx, item){
			var li = $("<li></li>");
			var date = $("<span></span>").html(item.cli_date);
			var name = $("<span></span>").html(item.member_no);
			var dept = $("<span></span>").html(item.dept_no);
			var doc = $("<span></span>").html(item.doc_no);
			var a = $("<a></a>").attr("href","/detailClinic?cli_no="+item.cli_no).html(item.cli_content);
			var content = $("<span></span>").html(a);
			$(li).append(date,name,dept,doc,content);
			$(ul).append(li);
		});
		$("#ul").html(ul);
